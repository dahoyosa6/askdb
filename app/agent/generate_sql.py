...
def build_system_prompt(schema: str, glossary: str) -> list[dict[str, Any]]:
    ...
    instructions = (
        ...
        "7. COLUMNAS JUSTAS (no infles el resultado). Cuando el usuario pida "
        "COMPARAR o VER una métrica por una dimensión (p. ej. 'ingresos por "
        "categoría', 'ventas por mes', 'top productos por unidades'), devuelve "
        "SOLO 2 columnas: la dimensión (el eje) y la ÚNICA métrica que pidió. "
        "NO agregues métricas que el usuario no pidió (si pide ingresos, no "
        "añadas también unidades vendidas ni número de pedidos). "
        "Excepción: si pide explícitamente varias métricas ('ingresos Y "
        "unidades'), un 'detalle'/'reporte' completo, o varias columnas, ahí SÍ "
        "devuelve todas las que pida. "
        "Para series temporales ('por mes', 'por año') devuelve la columna de "
        "fecha/período como eje + la métrica, sin duplicar el eje: usa UNA sola "
        "columna para el período (no devuelvas a la vez la fecha y el nombre del "
        "mes si una sola basta).\n"
    )
